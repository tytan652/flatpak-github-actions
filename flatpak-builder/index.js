 ...
 */
const build = async (manifest, manifestPath, bundle, buildBundle, repositoryUrl, repositoryName, buildDir, localRepoName, cacheBuildDir, cacheKey, cacheHitKey, arch, mirrorScreenshotsUrl) => {
...
const run = async (
  ...
  parseManifest(manifestPath)
    ...
    .then((manifest) => {
      return build(manifest, modifiedManifestPath, bundle, buildBundle, repositoryUrl, repositoryName, buildDir, localRepoName, cacheBuildDir, cacheKey, cacheHitKey, arch, mirrorScreenshotsUrl)
    })
    .then(() => {
      if (dbusSession) {
        dbusSession.kill()
